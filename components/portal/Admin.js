...
export default function Admin({ project, user, onRefresh }) {
  ...
  const deletePost = async (i) => {
    if (!confirm('¿Eliminar esta nota de bitácora?')) return
    const newPosts = postsDB.filter((_,idx)=>idx!==i)
    const ok = await api({posts: newPosts})
    if (ok) showToast('Nota eliminada')
  }

  const deleteNota = async (i) => {
    if (!confirm('¿Eliminar esta nota?')) return
    const newNotes = notesDB.filter((_,idx)=>idx!==i)
    const ok = await api({notes: newNotes})
    if (ok) showToast('Nota eliminada')
  }
  ...
  return (
    <div>
      <div className="section-hero" style={{marginBottom:16}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px 16px'}}>
          {[['nombre','Nombre'],['cliente','Cliente'],['ubicacion','Ubicación'],['arquitecto','Arquitecto'],['superficie','Superficie m²'],['niveles','Niveles'],['inicio','Fecha inicio'],['entrega','Entrega estimada'],['etapa_actual','Etapa actual']].map(([k,label])=>(
            <div key={k}>
              ...
              <input style={inputStyle} value={info[k]} onChange={e=>setInfo(i=>({...i,[k]:e.target.value}))}/>
            </div>
          ))}
        </div>
        <button className="btn-submit" style={{maxWidth:220,marginTop:16}} onClick={()=>api(info).then(ok=>ok&&showToast('Información guardada'))} disabled={saving}>Guardar información</button>
      </div>

      {/* ── ETAPAS ───────────────────────────────────── */}
      <div className="card" style={{marginBottom:12}}>
        <div className="card-title">Avance por etapa (%)</div>
        <p style={{fontSize:12,fontWeight:300,color:'var(--g400)',marginBottom:12}}>Mueve el slider para actualizar. Arrastra <span style={{color:'var(--ink)'}}>⠿</span> para reordenar.</p>
        <div style={{overflowY:'auto',maxHeight:340,marginBottom:10}}>
          {etapas.map((e,i)=>(
            <div key={i} draggable onDragStart={()=>{dragIndex.current=i}}
              ...
              style={{background:'var(--paper)',padding:12,marginBottom:8,cursor:'grab'}}>
              {/* Fila 1: drag + nombre + periodo + estatus */}
              <div style={{display:'grid',gridTemplateColumns:'20px 1fr 1fr auto',gap:8,marginBottom:10,alignItems:'start'}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:10,paddingLeft:28}}>
                <span style={{fontSize:11,color:'var(--g500)',flex:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{e.nombre||'Etapa'}</span>
      ...
      <div className="card" style={{marginBottom:12}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px 16px',marginBottom:12}}>
          ...
          <div>
            <label className="form-label" style={{marginBottom:6,display:'block'}}>Por pagar (MXN)</label>
            <input style={{...inputStyle,background:'var(--g100)'}} type="number" value={porPagar} readOnly/>
          </div>
        </div>
      ...
      <div className="card" style={{marginBottom:12}}>
        ...
        {photosDB.length>0&&(
          <div style={{overflowY:'auto',maxHeight:280,marginTop:8}}>
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:4,marginBottom:12}}>
              {photosDB.map((f,i)=>(
                <div key={i} style={{aspectRatio:'1',overflow:'hidden',background:'var(--g100)',position:'relative'}}>
                  ...
                  <button onClick={async()=>{ if(!confirm('¿Eliminar esta foto?')) return; const newPhotos=photosDB.filter((_,idx)=>idx!==i); const ok=await api({photos:newPhotos}); if(ok) showToast('Foto eliminada') }} style={{position:'absolute',top:4,right:4,width:20,height:20,borderRadius:'50%',background:'rgba(12,12,12,.7)',border:'none',color:'#fff',fontSize:10,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>✕</button>
                </div>
              ))}
            </div>
          </div>
        ...
        <button className="btn-submit" style={{maxWidth:180}} onClick={async()=>{
          ...
          for(const file of fotoFiles){ const result=await uploadFile(file,p.id,'project-photos'); if(result?.url)newPhotos.unshift({nombre:file.name,url:result.url,fecha:new Date().toLocaleDateString('es-MX')}) }
      ...
      <div className="card" style={{marginBottom:12}}>
        ...
        <textarea style={{...inputStyle,resize:'vertical',marginBottom:12}} placeholder="Describe el avance de esta semana..." value={postText} onChange={e=>setPostText(e.target.value)} rows={4}/>
        <button className="btn-submit" style={{maxWidth:180,marginBottom:16}} onClick={async()=>{
        ...
        {postsDB.length===0&&<p style={{fontSize:12,color:'var(--g400)'}}>Sin notas publicadas.</p>}
        {postsDB.map((post,i)=>(
          <div key={i} style={{padding:'10px 0',borderBottom:'1px solid var(--border)',display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8}}>
            <div style={{flex:1}}>
      ...
      <div className="card" style={{marginBottom:12}}>
        ...
        <textarea style={{...inputStyle,resize:'vertical',marginBottom:12}} placeholder="Mensaje directo al cliente..." value={notaText} onChange={e=>setNotaText(e.target.value)} rows={3}/>
        <button className="btn-submit" style={{maxWidth:220,marginBottom:16}} onClick={async()=>{
          ...
          if(ok){ setNotaText(''); showToast('Nota enviada al cliente'); sendNotaArqEmail({clientEmail:p.client_email,arquitectoNombre:user.name||user.email,arquitectoEmail:user.email,proyecto:p.nombre,cliente:p.cliente,nota:notaText.trim()}) }
        }} disabled={saving}>Enviar nota al cliente</button>
        {notesDB.length===0&&<p style={{fontSize:12,color:'var(--g400)'}}>Sin notas enviadas.</p>}
        {notesDB.map((n,i)=>(
      ...
      <div className="card" style={{marginBottom:12}}>
        ...
        <div className="form-field">
          <label className="form-label">Subir archivo (opcional)</label>
