...
const EMAILJS_TEMPLATE = 'template_jznns8p'
...
export default function MasterPanel({ onImpersonate, onLogout }) {
  ...
  const sendBroadcast = async () => {
    ...
    try { await loadEmailJS() } catch (e) {
      setBSending(false)
      setBResult({ ok: false, sent: 0, failed: recipients.length })
      return
    }
    ...
    for (const u of recipients) {
      try {
        ...
        console.error('EmailJS error:', u.email, JSON.stringify(err))
        failed++
      }
