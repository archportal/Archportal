...
export default function LoginModal({ onClose, onLogin, onRegister, lang }) {
  ...
  const t = lang === 'en'
    ? {
        title: 'Sign in',
        sub: 'Sign in to your project portal',
        user: 'Email',
        pass: 'Password',
        btn: 'Enter portal',
        loading: 'Loading...',
        err_credentials: 'Incorrect username or password',
        ...
        forgotSub: 'Enter your email and we will send your access information.',
        forgotBtn: 'Send',
        forgotSentMsg: 'If you have an architect account, you will receive an email shortly.',
        cancel: 'Cancel',
      }
    : {
        title: 'ArchPortal',
        sub: 'Ingresa a tu portal de proyecto',
        user: 'Usuario',
        pass: 'Contraseña',
        btn: 'Entrar al portal',
        loading: 'Cargando...',
        err_credentials: 'Usuario o contraseña incorrectos',
        err_network: 'Error de conexión. Intenta de nuevo.',
        back: 'Volver al inicio',
        noAccount: '¿No tienes cuenta?',
        register: 'Regístrate',
        forgot: '¿Olvidaste tu contraseña?',
        forgotTitle: 'Recuperar acceso',
        forgotSub: 'Ingresa tu correo de arquitecto y te enviaremos tu información de acceso.',
        forgotBtn: 'Enviar',
        forgotSentMsg: 'Si tienes una cuenta de arquitecto recibirás un correo en breve.',
        cancel: 'Cancelar',
      }
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        // 5xx u otros errores no relacionados con credenciales
        setError(t.err_network)
        return
      }

      if (data.user.role === 'master') { onLogin(data.user, [], null); return }
      if (data.user.role === 'cli') { onLogin(data.user, [], data.client_project); return }
  ...
  const handleForgot = async () => {
    ...
    try {
      ...
      // Por seguridad, mostramos el mismo mensaje aunque falle (no revelar si el email existe)
      setForgotSent(true)
    } finally {
      ...
      <div className="modal" style={{ maxWidth: 420 }}>
        <div className="modal-body">
          ...
          <form onSubmit={handleLogin}>
            <div className="form-field"><label className="form-label">{t.user}</label><input type="email" className="form-input" placeholder="tu@email.com" value={email} onChange={e => setEmail(e.target.value)} required autoComplete="email" /></div>
            <div className="form-field"><label className="form-label">{t.pass}</label><input type="password" className="form-input" placeholder="..." value={password} onChange={e => setPassword(e.target.value)} required autoComplete="current-password" /></div>
            {error && <p className="form-error">{error}</p>}
            <button type="submit" className="btn-submit" disabled={loading} style={{ marginTop: 24 }}>{loading ? t.loading : t.btn}</button>
          </form>
          ...
          <p style={{ marginTop: 16, fontSize: 12, fontWeight: 300, color: 'var(--g400)', textAlign: 'center' }}>{t.noAccount}{' '}<button type="button" onClick={onRegister} style={{ background: 'none', border: 'none', color: 'var(--ink)', cursor: 'pointer', textDecoration: 'underline', fontSize: 'inherit', fontFamily: 'inherit', padding: 0 }}>{t.register}</button></p>
